...
| `dto/user/ChangeEmailRequest.java` | New — change-email request DTO |
| `models/User.java` | Added `pendingEmail` and `adminDeactivated` fields |
| `repos/VerificationTokenRepository.java` | Used for `email_change` token lifecycle |
| `resources/templates/email-change-verification.html` | New — Thymeleaf email template |
| `services/EmailService.java` | Added `sendEmailChangeVerificationEmail()` |
| `services/UserProfileService.java` | Added `requestEmailChange()` method |
| `controllers/UserProfileController.java` | Added `POST /api/v1/profile/change-email` |
| `controllers/AuthController.java` | Added `GET /api/v1/auth/verify-email-change` |

---

## Change Email Flow (added in v28.1)

See [29CHANGE_EMAIL_FEATURE.md](28.1CHANGE_EMAIL_FEATURE.md) for the full specification.

**New endpoint summary:**

| Method | Path | Auth | Description |
|--------|------|------|-------------|
| `POST` | `/api/v1/profile/change-email` | `ROLE_USER` | Initiate email change (password + new email) |
| `GET`  | `/api/v1/auth/verify-email-change?token=xxx` | Public | Confirm the new email address |
