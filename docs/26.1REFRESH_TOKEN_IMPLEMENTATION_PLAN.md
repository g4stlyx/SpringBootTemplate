...
1. **Access Token (JWT)**
   ...
   - **Contents**: userId, role, username, adminLevel (for admins)
...
This system supports two user types:
- `user` - Regular users
...
- `role`: Filter by user type (user, admin)
- `userId`: Filter by specific user ID
- `isRevoked`: Filter by revocation status (true/false)
- `ipAddress`: Filter by IP address

**Example**: `GET /api/v1/admin/refresh-tokens?role=user&isRevoked=false`
...
**Example**: `GET /api/v1/admin/refresh-tokens/user/5?role=user`
...
**Example**: `PUT /api/v1/admin/refresh-tokens/revoke-all?userId=5&role=user`
...
2. Call `/admin/refresh-tokens?role=user` - view user tokens
